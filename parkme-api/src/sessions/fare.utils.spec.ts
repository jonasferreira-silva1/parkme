...
import { calcularTarifa, calcularMinutos, temPrecoDinamico } from './fare.utils';
...
describe('calcularTarifa — testes unitários', () => {
  ...
  it('lida com 0 minutos (mínimo de 1h é aplicado)', () => {
    expect(calcularTarifa(0, 15, 0.5)).toBe(15.00);
  });
  ...
  it('PROPRIEDADE: tarifa nunca é negativa', () => {
    fc.assert(
      fc.property(
        ...
        fc.float({ min: 0, max: 100 }),      // Preço: R$0 a R$100/h
        fc.float({ min: 0, max: 1 }),        // Ocupação: 0% a 100%
